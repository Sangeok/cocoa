...
export class WithdrawService {
    ...
    try {
      const fromFees =
        from === 'upbit'
          ...
          : await this.feeClient.getBinanceFees();
  ...
  private async calculateKoreaToGlobalPaths(
    ...
    // 수익률 기준으로 정렬하고 필터링
    return paths
      .filter((path) => path.estimatedReceiveAmount > 0)
  ...
  private async getCurrentPrice(coin: string, exchange: Exchange): Promise<number> {
    try {
      if (exchange === 'binance') {
        if (coin === 'USDT') return 1;
        const key = createTickerKey(exchange, 'USDT', coin);
        const tickerStr = await this.redisService.get(key);
        if (!tickerStr) throw new Error(`No price data found for ${coin} on ${exchange}`);
      ...
      this.logger.error(`Failed to get price for ${coin} on ${exchange}: ${error.message}`);
      throw error;
    }
  }
