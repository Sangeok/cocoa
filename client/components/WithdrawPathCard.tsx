...
import { formatKRW, formatCrypto } from '@/dto/withdraw.dto'
...
export default function WithdrawPathCard({ path, index }: WithdrawPathCardProps) {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg p-4 space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <span className={clsx(
          "font-medium",
          path.profitRate > 0 ? "text-green-500 dark:text-green-400" : "text-red-500 dark:text-red-400"
        )}>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          <p className="text-sm text-gray-500 dark:text-gray-400">출금 수수료</p>
        ...
        <div>
          <p className="text-sm text-gray-500 dark:text-gray-400">예상 수령액</p>
          <p className="text-gray-900 dark:text-white">
      ...
      {/* Steps */}
      <div>
        <p className="text-sm text-gray-500 dark:text-gray-400 mb-2">송금 단계</p>
        <ul className="space-y-1">
          {path.steps.map((step, stepIndex) => (
            <li key={stepIndex} className="text-gray-900 dark:text-white text-sm">
              {stepIndex + 1}. {step}
            </li>
          ))}
        </ul>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-2 text-sm">
          <div className="bg-gray-50 dark:bg-gray-900/50 p-2 rounded">
            <span className="text-gray-500 dark:text-gray-400">구매한 코인</span>
            <p className="text-gray-900 dark:text-white font-medium">{path.coin}</p>
          </div>
          <div className="bg-gray-50 dark:bg-gray-900/50 p-2 rounded">
            <span className="text-gray-500 dark:text-gray-400">구매 수량</span>
            <p className="text-gray-900 dark:text-white font-medium">{formatCrypto(path.amount)}</p>
          </div>
          <div className="bg-gray-50 dark:bg-gray-900/50 p-2 rounded">
