...
export default function NotificationModal({
  ...
  const getMessage = async (messageId: number) => {
    const response = await ClientAPICall.get(
      API_ROUTES.MESSAGE.GET.url.replace(":messageId", messageId.toString())
    );
    return response.data;
  };
  ...
  const handleNotificationClick = async (notification: Notification) => {
    ...
    } else if (notification.type === "NEW_ADMIN_MESSAGE") {
      const message = await getMessage(notification.targetId);
