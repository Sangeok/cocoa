...
export default function Banner({ position }: BannerProps) {
  ...
  // 배너 클릭 핸들러
  const handleBannerClick = (forwardUrl: string) => {
  ...
  return (
    <>
      <DesktopBanner
        ...
        onClick={() => desktopBanner?.forwardUrl && handleBannerClick(desktopBanner.forwardUrl)}
      />
      <TabletBanner
        imageUrl={tabletBanner?.imageUrl}
        onClick={() => tabletBanner?.forwardUrl && handleBannerClick(tabletBanner.forwardUrl)}
      />
      <MobileBanner
        imageUrl={mobileBanner?.imageUrl}
        onClick={() => mobileBanner?.forwardUrl && handleBannerClick(mobileBanner.forwardUrl)}
      />
    </>
  );
}
